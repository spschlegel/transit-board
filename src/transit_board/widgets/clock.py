...
def draw_clock(image: Image.Image, font_path: str | None = None) -> None:
    ...
    # widths and glyph artifacts (confirmed by inspection). The info column
    # (INFO_W=44px) is wide enough for the time at 16px ("23:45" is 36px
    # there); the date stays at the native 8px.
    ...
    dx = x0 + max(0, (w - dw) // 2)
    draw.text((dx, y0 + 17), date_str, font=font_date, fill=layout.DIM)
